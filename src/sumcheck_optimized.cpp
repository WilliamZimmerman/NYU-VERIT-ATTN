...
public:
    ...
    FieldElement evaluate(const std::vector<FieldElement>& point) const {
        ...
        std::vector<FieldElement> row_indicator(max_row_vals);
        std::vector<FieldElement> col_indicator(max_col_vals);
        //Precompute Multiplications of MLE for speed
        for(int r_idx = 0; r_idx < max_row_vals; r_idx++)
        {
            FieldElement current_row_indicator(1);
            for (int bit = 0; bit < log_rows; bit++) {
                        int v_bit = (r_idx >> bit) & 1;
                        current_row_indicator = current_row_indicator * ((FieldElement(1) - point[bit]) * FieldElement(1 - v_bit) + point[bit] * FieldElement(v_bit));
                    }
            row_indicator[r_idx] = current_row_indicator;
        }

        for(int c_idx = 0; c_idx < max_row_vals; c_idx++)
        {
            FieldElement current_col_indicator(1);
            for (int bit = 0; bit < log_rows; bit++) {
                        int v_bit = (c_idx >> bit) & 1;
                        current_col_indicator = current_col_indicator * ((FieldElement(1) - point[log_rows + bit]) * FieldElement(1 - v_bit) + point[log_rows + bit] * FieldElement(v_bit));
                    }
            col_indicator[c_idx] = current_col_indicator;
        }
        ...
        for (int row_bits = 0; row_bits < max_row_vals; row_bits++) {
            for (int col_bits = 0; col_bits < max_col_vals; col_bits++) {
                ...
                FieldElement indicator = row_indicator[row_bits] * col_indicator[col_bits];
...
public:
    ...
    ThalerAttentionProver(
        ...
        const std::vector<std::vector<FieldElement>>& Q, //weight for Q
        const std::vector<std::vector<FieldElement>>& K, //weight for Q
    ...
    FieldElement evaluate_reduced_polynomial(const std::vector<FieldElement>& pqh_point) const {
        ...
        std::vector<FieldElement> xip = r_i; 
        xip.insert(xip.end(), p.begin(), p.end()); //Xi MLE prep

        std::vector<FieldElement> qph = p; 
        qph.insert(qph.end(), h.begin(), h.end()); //Prepare Query Matrix at p and h MLE

        std::vector<FieldElement> xjq = r_j; xjq.insert(xjq.end(), //Xj MLE prep
        q.begin(), q.end());

        std::vector<FieldElement> kqh = q;
        kqh.insert(kqh.end(), h.begin(), h.end());//Prepare Key matrix at q and h

        return X_mle.evaluate(xip) * Q_mle.evaluate(qph) * X_mle.evaluate(xjq) * K_mle.evaluate(kqh); //evaluate the MLEs at these points and multiply them
    }
...
int main() {
    try {
        ...
        std::cout <<"completed claimed sum" << std::endl;
